...
void LogAppender::setFormatter(LogFormatter::ptr val) {
    MutexType::Lock lock(m_mutex);//多线程写的时候不能让别人读或者写
    m_formatter = val;
    if(m_formatter) {
        m_hasFormatter = true;
    } else {
...
LogFormatter::ptr LogAppender::getFormatter() {
    MutexType::Lock lock(m_mutex);//多线程读的时候不能让别人读或者写
    return m_formatter;
}
...
void Logger::setFormatter(const std::string& val) {
    zy::LogFormatter::ptr new_val(new zy::LogFormatter(val));
    if(new_val->isError()) {
...
//对下面配置文件的初始化做偏特化，根据log.yml内容进行解析
template<>
